"""
This is the graphical user interface, the face of this game. Any function that needs to call back to logic controllers
does so by calling back to the main module for proper calculating. All the main windows are implemented but not fully
functional, though as of now the game is functional. The window sizing should be based on the system its running on and
displayed as a full screen window. I think the window layouts should should maintain regardless of screen size but
that has not been tested.

FIXME: Button highlightbackground is not working on all buttons
"""
...
class Interface(Frame):
    def __init__(self, master=None):
        ...
        self.character = main.CharacterController()
        self.player = {}
        self.opponent = {}
    ...
    def init_window(self):
        ...
        main_container = Frame(self.main_frame, bg="wheat4", width=self.width, height=self.height,
                               highlightbackground="black", highlightthickness=4)
        main_container.grid_columnconfigure((1, 4), weight=1, minsize=100)
        main_container.grid_columnconfigure((2,3), weight=2)
        main_container.grid_rowconfigure(2, weight=2, minsize=50)
        main_container.grid_rowconfigure((1, 3), weight=1, minsize=50)
        ...
        graphic_container.grid(row=2, column=3)
        control_container.grid(row=2, column=2)
        pad_frame1.grid(row=1, column=1, columnspan=4, sticky=N)
        pad_frame2.grid(row=3, column=1, columnspan=4, sticky=S)
        ...
        new_button = Button(control_button_frame, text="New\nGame", width=9, height=4, bg="wheat4",
                            relief=RAISED, bd=3, highlightbackground="darkolivegreen", activebackground="burlywood3",
                            font=button_font, command=lambda: new_game(self))
        cont_button = Button(control_button_frame, text="Continue\nGame", width=9, height=4, bg="wheat4",
                             relief=RAISED, bd=3, highlightbackground="darkolivegreen", activebackground="burlywood3",
                             font=button_font, command=lambda: continue_game(self), state="disabled")
        load_button = Button(control_button_frame, text="Load\nGame", width=9, height=4, bg="wheat4",
                             relief=RAISED, bd=3, highlightbackground="darkolivegreen", activebackground="burlywood3",
                             font=button_font, command=lambda: load_game(self), state="disabled")
        exit_button = Button(control_button_frame, text="Exit\nGame", width=9, height=4, bg="wheat4",
                             relief=RAISED, bd=3, highlightbackground="darkolivegreen", activebackground="burlywood3",
                             font=button_font, command=lambda: exit())
    ...
    # Not Implemented yet
    def world_window(self, prompt1=None, prompt2=None):
        # Fixme: pass in self
        def move_temo(self):
            playerlvl = self.player["level"]
            event = self.progress.firststrike(playerlvl)
            main_container.after(0, self.combat_window(event[0],event[1], event[2]))
            main_container.destroy()
        ...
        player_name.set(str(self.player["name"]))
        player_lvl = StringVar()
        player_lvl.set(str(self.player["level"]))
        player_health = self.player["health"]
        player_image = "/race_elf.png"
        general_prompt = StringVar()
        if prompt1 is None:
            general_prompt.set("As " + str(self.player["name"] + " rests..."))
        else:
            general_prompt.set(prompt1)

        # Image configurations
        character_img = PhotoImage(file=directory + "/images" + player_image)
        arrow_img1 = PhotoImage(file=directory + "/images/left_arrow.png")
        arrow_img2 = PhotoImage(file=directory + "/images/right_arrow.png")
        arrow_img3 = PhotoImage(file=directory + "/images/up_arrow.png")
        arrow_img4 = PhotoImage(file=directory + "/images/down_arrow.png")
        button_bg_img1 = PhotoImage(file=directory + "/images/button_bg1.png")
        button_bg_img2 = PhotoImage(file=directory + "/images/button_bg2.png")
        ...
        player_name = Label(player_info, textvar=player_name, font=info_font, bg="burlywood3")
        player_level = Label(player_info, textvar=player_lvl, font=info_font, bg="burlywood3")
        ...
        character.image = reduced_character_img
        ...
        center_button = Button(button_frame, text="Center", bg="black", image=reduced_button_bg_img2, relief=RAISED,
                               bd=3, activebackground="burlywood3")
        center_button.image = reduced_button_bg_img2
        button_background = Label(button_frame, bg="black", width=25, height=15)
        inventory_button = Button(button_frame, text="Inventory", width=14, height=2, font=button_font, bg="wheat4",
                                  relief=RAISED, bd=3,highlightbackground="darkolivegreen", activebackground="burlywood3")
        log_button = Button(button_frame,  text="Quest Log",  width=14, height=2, font=button_font, bg="wheat4",
                            relief=RAISED, bd=3,highlightbackground="darkolivegreen", activebackground="burlywood3")
        skills_button = Button(button_frame, text="Skill Tree", width=14, height=2, font=button_font, bg="wheat4",
                               relief=RAISED, bd=3,highlightbackground="darkolivegreen", activebackground="burlywood3")
        game_prompt = Label(prompt_frame, textvar=general_prompt, bg="burlywood3", font=prompt_font)
    ...
    def creation_window(self):
        ...
        # the main module's CharacterController for use in the type_controller function. It uses the variable
        ...
        def attribute(self, direction):
            ...
            attribute_results = self.character.type_controller(attribute_selection[choice])
        ...
        # module's CharacterController for use in the builder function. See Character module for more details on
        # character stats.
        def build_character(self):
            name_length = len(name_input.get())
            attributes = [name_input.get(), character_type_text.get(), character_race_text.get(),1, 100,
                          int(attack_points.get()), int(defense_points.get()), int(magic_points.get())]
            if name_length == 0:
                return
            else:
                if int(attribute_points.get()) != 0:
                    ...
                    self.player = self.character.builder(attributes)
                    main_container.after(0, self.world_window())
                    main_container.destroy()
        ...
        attributes_container = Frame(main_container, bg="burlywood4",
                                     highlightthickness=3, highlightbackground="black")
        ...
        attributes_container.grid_rowconfigure(6, weight=2, minsize=275)
        ...
        pad_frame2.grid_rowconfigure((1, 3), minsize=5)
        main_container.pack(fill=BOTH, expand=TRUE)
        character_frame.grid(row=2, column=3)
        attributes_container.grid(row=2, column=2)
        pad_frame1.grid(row=1, column=1, columnspan=4, sticky=N)
        pad_frame2.grid(row=3, column=1, columnspan=4, sticky=S)
        ...
        finish_button = Button(attributes_button_frame, text="Finalize Character", width=20, height=3,
                               font=button_font, bg="darkolivegreen", relief=RAISED, bd=3,
                               highlightbackground="darkolivegreen" , activebackground="burlywood3",
                               command=lambda: build_character(self))
    ...
    # essentially the bulk of the game at this point.
    def combat_window(self, opp_stat, prompt, fs):
        # This sub function handles spell casting
        def casting_frame(self):
            # This sub sub function, idk what else to call it lol, processes the outcome of the chosen spell
            def select(self):
                item_selection = var.get()
                results = self.progress.action_controller(self.player, self.opponent, 3, item_selection)
                self.player = results[0]
                self.opponent = results[1]
                player_action_prompt1_text.set(results[2])
                player_action_prompt2_text.set(results[3])
                opp_hp_bar["value"] = self.opponent["health"]
                player_mp_bar["value"] -= 5
                player_effects_title_text.set("Attributes               ")
                self.spell_frame.destroy()
                spell_active.set(False)
                if item_active.get() is True:
                    self.item_frame.destroy()
                    item_active.set(False)
                else:
                    pass
            ...
            # This sub sub function creates radiobutton widgets for spell_subframe1 and aligns
            # them within that sub frame
            def spell_sorter():
                frame_height = 211
                row_count = 2
                spell_count = len(player_spells)
                row_min = frame_height / spell_count
                button_array = {}
                button_list = []
                lst_pos = 0
                for i in player_spells:
                    item = Radiobutton(spell_subframe1, text=i, variable=var, value=i, bg="wheat3", font=info_font2)
                    button_list.append(item)
                for i in player_spells:
                    button_array[i] = button_list[lst_pos]
                    lst_pos += 1
                while spell_count != 0:
                    for i in button_array:
                        spell_subframe1.grid_rowconfigure(row_count, minsize=row_min)
                        button_array[i].grid(row=row_count, column=1, sticky=W)
                        row_count += 1
                        spell_count -= 1
            ...
            # Variables
            var = StringVar()
            player_effects_title_text.set("Spells                   ")
            player_spells = self.player["spells"]
            spell_active.set(True)

            # Spell frame and sub frame configurations
            self.spell_frame = Frame(player_effects, width=190, height=265, bg="burlywood3")
            self.spell_frame.grid_columnconfigure(2, weight=2, minsize=180)
            self.spell_frame.grid_columnconfigure((1, 3), minsize=5)
            self.spell_frame.grid_rowconfigure(2, weight=2)
            self.spell_frame.grid_rowconfigure(4, weight=1)
            self.spell_frame.grid_rowconfigure((1, 5), minsize=5)
            self.spell_frame.grid_rowconfigure(3, minsize=2)
            self.spell_frame.grid(row=4, column=1, rowspan=15, columnspan=3)
            spell_subframe1 = Frame(self.spell_frame, width=180, height=215, bg="wheat3", highlightthickness=2,
                                    highlightbackground="black")
            spell_subframe1.grid_columnconfigure(1, minsize=175)
            spell_subframe1.grid_columnconfigure(2, minsize=2)
            spell_subframe1.grid_rowconfigure((1, 50), minsize=2)
            spell_subframe2 = Frame(self.spell_frame, width=180, height=35, bg="burlywood3", highlightthickness=2,
                                    highlightbackground="black")
            spell_subframe1.grid(row=2, column=2)
            spell_subframe2.grid(row=4, column=2)

            # Spell_subfame1 widgets
            select_button = Button(spell_subframe2, text="Select", width=15, font=button_font, bg="wheat4", bd=3,
                                   relief=RAISED, activebackground="burlywood3", command=lambda: select(self))
            select_button.pack(fill=BOTH)

            # Holding Frame update
            player_effects.update()
            spell_sorter()

        # This sub function handles user inventory
        def inventory(self):

            # Handles outcome of chosen item use
            def select(self):
                item_selection = var.get()
                result_list = self.character.use_item(self.player, item_selection)
                self.player = result_list[0]
                player_action_prompt1_text.set(result_list[1])
                player_action_prompt2_text.set(result_list[2])
                player_hp_bar["value"] = self.player["health"]
                player_effects_title_text.set("Attributes               ")
                self.item_frame.destroy()
                item_active.set(False)
                if spell_active.get() is True:
                    self.spell_frame.destroy()
                    spell_active.set(False)
                else:
                    pass
            ...
            # creates the radiobutton widgets for items_subframe1 and aligns them within that frame
            def item_sorter():
                frame_height = 211
                row_count = 2
                item_count = len(storage_items)
                row_min = frame_height / item_count
                button_array = {}
                button_list = []
                lst_pos = 0
                for i in storage_items:
                    item = Radiobutton(item_subframe1,
                                       text=str(stored_quantity[storage_items.index(i)]) + "    " + str(i),
                                       variable=var, value=i, bg="wheat3", font=info_font2)
                    button_list.append(item)
                for i in storage_items:
                    button_array[i] = button_list[lst_pos]
                    lst_pos += 1
                while item_count != 0:
                    for i in button_array:
                        item_subframe1.grid_rowconfigure(row_count, minsize=row_min)
                        button_array[i].grid(row=row_count, column=1, sticky=W)
                        row_count += 1
                        item_count -= 1
                player_effects.update()
            ...
            # Variables
            var = StringVar()
            player_effects_title_text.set("Inventory                ")
            player_storage_check = self.character.storage_check()
            storage_items = player_storage_check[0]
            stored_quantity = player_storage_check[1]
            item_active.set(True)

            # item frame and sub frames configurations
            self.item_frame = Frame(player_effects, width=190, height=265, bg="burlywood3")
            self.item_frame.grid_columnconfigure(2, weight=2, minsize=180)
            self.item_frame.grid_columnconfigure((1, 3), minsize=5)
            self.item_frame.grid_rowconfigure(2, weight=2)
            self.item_frame.grid_rowconfigure(4, weight=1)
            self.item_frame.grid_rowconfigure((1, 5), minsize=5)
            self.item_frame.grid_rowconfigure(3, minsize=2)
            self.item_frame.grid(row=4, column=1, rowspan=15, columnspan=3)
            item_subframe1 = Frame(self.item_frame, width=180, height=215, bg="wheat3", highlightthickness=2,
                                   highlightbackground="black")
            item_subframe1.grid_columnconfigure(1, minsize=175)
            item_subframe1.grid_columnconfigure(2, minsize=2)
            item_subframe1.grid_rowconfigure((1, 50), minsize=2)
            item_subframe2 = Frame(self.item_frame, width=180, height=35, bg="burlywood3", highlightthickness=2,
                                   highlightbackground="black")
            item_subframe1.grid(row=2, column=2)
            item_subframe2.grid(row=4, column=2)

            # item_subframe2 widgets
            select_button = Button(item_subframe2, text="Select", width=15, font=button_font, bg="wheat4", fg="white",
                                   bd=3, relief=RAISED, activebackground="burlywood3", command=lambda: select(self))
            select_button.pack(fill=BOTH)
            # Holding frame update
            player_effects.update()
            item_sorter()
        ...
        first_turn = fs
        item_active = BooleanVar()
        spell_active = BooleanVar()
        ...
        player_effects_title_text = StringVar()
        ...
        main_container = Frame(self.main_frame, bg="darkolivegreen")
        main_container.grid_columnconfigure((1,3), weight=1, minsize=5)
        main_container.grid_columnconfigure(2, weight=2, minsize=200)
        main_container.grid_rowconfigure((1,2), weight=2, minsize=380)
        opponent_frame = Frame(main_container, bg="burlywood4",
                               highlightbackground="black", highlightthickness=8)
        opponent_frame.grid_columnconfigure((1, 5), weight=1, minsize=100)
        opponent_frame.grid_columnconfigure((2, 3, 4), minsize=300)
        opponent_frame.grid_rowconfigure((1, 5), weight=1, minsize=25)
        opponent_frame.grid_rowconfigure((2,3,4), minsize=40)
        player_frame = Frame(main_container, bg="burlywood4",
                             highlightbackground="black", highlightthickness=8)
        player_frame.grid_columnconfigure((1, 5), weight=1, minsize=100)
        player_frame.grid_columnconfigure((2, 3, 4), minsize=300)
        player_frame.grid_rowconfigure((1, 5), weight=1, minsize=25)
        player_frame.grid_rowconfigure((2,3,4), minsize=30)
        main_container.pack(fill=BOTH, expand=TRUE)
        opponent_frame.grid(row=1, column=2)
        player_frame.grid(row=2, column=2, sticky=N)
        ...
        opp_info.grid(row=2, column=3, sticky=N)
        opp_prompt.grid(row=3, column=1, columnspan=4, rowspan=2)
        opp_effects.grid(row=2, column=4, rowspan=3, columnspan=2)
        ...
        player_info.grid(row=2, column=3, sticky=N)
        player_prompt.grid(row=3, column=1, columnspan=4, rowspan=2)
        player_effects.grid(row=2, column=4, rowspan=3, columnspan=2)
        button_frame.grid(row=5, column=1, columnspan=4, sticky=S)
        ...
        player_effects_title = Label(player_effects, textvariable=player_effects_title_text, font=effects_font,
                                     bg="burlywood3")
        ...
        attack_button = Button(button_frame, text="Attack", width=15, height=2, font=button_font, bg="wheat4",
                               relief=RAISED, bd=3, activebackground="burlywood3", command=lambda: turn(self, 1))
        defend_button = Button(button_frame, text="Defend", width=15, height=2, font=button_font, bg="wheat4",
                               relief=RAISED, bd=3, activebackground="burlywood3", command=lambda: turn(self, 2))
        spells_button = Button(button_frame, text="Cast", width=15, height=2, font=button_font, bg="wheat4",
                               relief=RAISED, bd=3, activebackground="burlywood3", command=lambda: casting_frame(self))
        escape_button = Button(button_frame, text="Escape", width=15, height=2, font=button_font, bg="wheat4",
                               relief=RAISED, bd=3, activebackground="burlywood3", command=lambda: turn(self, 4))
        item_button = Button(button_frame, text="Inventory", width=15, height=2, font=button_font, bg="wheat4",
                             relief=RAISED, bd=3, activebackground="burlywood3", command=lambda: inventory(self))
        ...
        if first_turn is True:
            ...
            main_container.update()
            time.sleep(1)
            main_container.after(0, lambda: next_turn(self))
        else:
            pass
